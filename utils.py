...
def load_data_sig(signal_path):
    sig_A, sig_B = load_signal(signal_path)
    sig_AB = np.concatenate((sig_A, sig_B), axis=0)

    return sig_AB

def load_signal(signal_path):
    input_sig = np.loadtxt(signal_path)
    w = int(input_sig.shape[0])
    w2 = 1021
    sig_A = input_sig[0:w2]
    sig_B = input_sig[w2+1:w]
